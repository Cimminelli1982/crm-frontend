...
import { FiPlus, FiTrash2, FiEdit2, FiSave, FiX, FiClock, FiFileText, FiInfo } from 'react-icons/fi';
...
const CompanyNotesModal = ({ isOpen, onRequestClose, company }) => {
  ...
  const handleCreateNote = async () => {
    if (!company?.company_id || !noteContent.trim()) return;
    ...
    try {
      ...
      // Associate the note with the company
    ...
    >
      <ModalContent>
        <Header>
          <Title>
            <FiFileText />
            Notes - <CompanyName>{company?.name}</CompanyName>
          </Title>
